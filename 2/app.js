...
axios.get('https://api.institutoalfa.org/api/songs').then(res => {
    songs = res.data.songs.map(song => {
        const li = document.createElement('li')
        li.setAttribute('class', "songItem")
        li.innerHTML = `
        
                        <span>01</span>
                <img src="https://api.institutoalfa.org/api/songs/image/${song.image.filename}" alt="Alan">
                <h5>
                    On My Way
    ...
    axios.get('https://api.institutoalfa.org/api/songs').then(res => {
        songs = res.data.songs.map(song => {
            ...
            document.getElementById('otra-cosa').appendChild(li)
        })
